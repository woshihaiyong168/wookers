...
class SshTwoController  extends  Controller {
    private $host ;
    private $user ;
    private $pass ;
    private $port ;
    ...
    public function __construct ( $host, $user, $pass, $port ) {
        ...
        //秘钥登录    需要填写$public_key(路径)、$private_key(路径)
        if ( $this->keyConnect ()->KeyAuthenticate () ) {
            return true;
        }
    }
...
//        $this->logAction ( "Authentication to {$this->host} failed" );
    ...
    public function keyConnect(){
        if($this->conn = ssh2_connect($this->host, $this->port, array('hostkey'=>'ssh-rsa'))){
            return $this;
        }
    }

    public function  KeyAuthenticate(){
        if(ssh2_auth_pubkey_file($this->conn,$this->user,$this->public_key,$this->private_key)){
            return $this;
        }
    }
...
//        if ( ! is_file ( $localFile ) ) throw new Exception ( "Local file {$localFile} does not exist" );
        if ( ! is_file ( $localFile ) ) echo "Local file {$localFile} does not exist";
...
//        $this->logAction ( "Receiving file $remoteFile as $localFile failed" );
        echo "Unable to get file to {$remoteFile}";
